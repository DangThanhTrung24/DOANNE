...
// kiểm tra đã hợp lệ chưa
  $("#description").keyup(function () {
    ...
    if (description == "") {
      ...
      if (length < minLength) {
        $("#description").addClass("is-invalid");
        $("#showErrorDescription").text("Mô tả phải lớn hơn 5 ký tự!");
        checkDescription = 10;
      } else {
...
// kiểm tra form đã nahapj hết chưa
function checkForm() {
  $("#code").keyup();
  $("#name").keyup();
  $("#price").keyup();
...
// chưa hợp lệ thì không hiện nút lưu
...
// hiển thị tất cả loại danh mục lên form
...
// hiển thị tất cả nhà cung cấp lên form
...
// load thông tin chi tiết sản phẩm được chọn
  $scope.load = function () {
    $http
