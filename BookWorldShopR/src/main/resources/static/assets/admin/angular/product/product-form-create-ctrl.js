...
$(document).ready(function () {
	...
	// kiểm tra đã nhập mã sản phẩm chưa
  $("#code").keyup(function () {
    var code = this.value;
    if (code == "") {
      ...
      if (length < minLength || length > maxLength) {
        $("#code").addClass("is-invalid");
        $("#showErrorCode").text("Nhập giá trị từ 4 đến 10 ký tự!");
        checkCode = 10;
      } else {
...
// kiểm tra đã nhập tên sản phẩm chưa
  $("#name").keyup(function () {
    var name = this.value;
    if (name == "") {
      ...
      if (length < minLength || length > maxLength) {
        $("#name").addClass("is-invalid");
        $("#showErrorName").text("Nhập giá trị từ 5 đến 100 ký tự!");
        checkName = 10;
      } else {
...
// đã nhập số lượng sản phẩm chưa
...
// nhập tên tìm kiếm chưa
  $("#nameSearch").keyup(function () {
    ...
    if (nameSearch == "") {
      ...
      if (length < minLength || length > maxLength) {
        $("#nameSearch").addClass("is-invalid");
        $("#showErrorNameSearch").text("Nhập giá trị từ 5 đến 100 ký tự!");
        checkNameSearch = 10;
      } else {
...
// chọn danh mục sản phẩm hay chưa
...
// thêm mô tả cho sản phẩm chưa
  $("#description").keyup(function () {
    ...
    if (description == "") {
      ...
      if (length < minLength) {
        $("#description").addClass("is-invalid");
        $("#showErrorDescription").text("Mô tả phải lớn hơn 50 ký tự!");
        checkDescription = 10;
      } else {
...
// nhập thông số cho sản phẩm chưa 
  $("#specification").keyup(function () {
    ...
    if (specification == "") {
      ...
      if (length < minLength) {
        $("#specification").addClass("is-invalid");
        $("#showErrorSpecification").text("Thông số phải lớn hơn 5 ký tự!");
        checkSpecification = 10;
      } else {
...
// kiểm tra form đã nhập đủ hết chưa
function checkForm() {
  $("#code").keyup();
  $("#name").keyup();
  $("#price").keyup();
...
// nếu chưa nhập đủ thì nút lưu sẽ bị vô hiệu hóa
...
// hiển thị tất cả danh mục sản phẩm lên form
...
// hiển thị tất cả nhầ cung cấp lên form
...
// thêm sản phẩm mới
  ...
  $scope.imageImage1Changed = function (files) {
    ...
    $http
      .post("/rest/upload/imageProducts", data, {
