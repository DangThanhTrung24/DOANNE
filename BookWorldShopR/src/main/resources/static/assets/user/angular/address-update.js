...
app.controller("address-add-ctrl", function ($scope, $http) {
  ...
  // Định nghĩa hàm load để tải dữ liệu địa chỉ cần cập nhật
  $scope.load = function () {
	  // Gửi yêu cầu GET đến địa chỉ /rest/address/update/ với mã địa chỉ cần cập nhật từ phần tử HTML có id là "demo"
    $http
      .get("/rest/address/update/" + $("#demo").val())
      .then((resp) => {
		  // Khi dữ liệu được trả về thành công, gán dữ liệu vào đối tượng form của $scope để hiển thị trong giao diện người dùng
  ...
  // Định nghĩa hàm updateAddress để cập nhật địa chỉ
  $scope.updateAddress = function () {
	  // Kiểm tra dữ liệu nhập liệu trước khi thực hiện cập nhật địa chỉ
    if(checkForm()){
		// Lấy giá trị của các trường tỉnh/thành, quận/huyện, phường/xã từ các phần tử HTML
        ...
        // Gửi yêu cầu PUT để cập nhật địa chỉ với mã địa chỉ từ phần tử HTML có id là "demo"
        $http.put(`/rest/address/form/` + $("#demo").val(), item).then((resp) => {      
			// Sau khi cập nhật thành công, làm mới trang để hiển thị thông tin mới     
            location.reload();
        });
    }
  }
