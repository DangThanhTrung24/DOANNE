...
import com.trungdt.validator.user.RegisterFormValidator;
	...
	public String handlerRegisterForm(Model model, @ModelAttribute("userRegister") @Validated UserRegister userRegister,
		...
		if (result.hasErrors()) {
			...
			if (userRegister.isConfirmTerm() == false) {
				...
				mailerService.queue(userRegister.getEmail(), "Xác nhận email!", "Mã xác nhận của bạn là: " + code + "<br> Hãy nhập vào và tiếp tục đăng kí");
